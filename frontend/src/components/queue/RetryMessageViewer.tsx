...
export default function RetryMessageViewer({ processorName, messageCount }: Props) {
  ...
  const fetchMessages = async (count: number = 100) => {
    setLoading(true);
    try {
      const result = await api.getRetryMessages(processorName, count);
      ...
      toast.error('Failed to load retry messages');
    } finally {
      setLoading(false);
    }
  };
